...
void LaneKeepingSystem<PREC>::run()
{
    ...
    while (ros::ok())
    {
        ...
        if(mFrame.empty() == true) {
            continue;
        }

        mLaneDetector -> imageProcessing(mFrame);
        int get_angle = mLaneDetector -> error_calculated;
        speedControl(mPID->getControlOutput(get_angle));
        drive(mPID->getControlOutput(get_angle) * 2);
